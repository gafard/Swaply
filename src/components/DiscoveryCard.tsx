...
export default function DiscoveryCard({ item, onSwipeRight, onSwipeLeft, isFront }: Props) {
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 overflow-hidden rounded-[38px] border-2 border-border bg-surface shadow-xl select-none"
    >
      {/* Dynamic Gesture Stickers */}
      <motion.div
        style={{ opacity: reserveOpacity, scale: reserveScale }}
        className="absolute left-6 top-8 z-40 rounded-2xl border-2 border-emerald-400 bg-gradient-to-r from-emerald-500 to-teal-500 px-5 py-2.5 text-base font-black uppercase tracking-widest text-white shadow-lg backdrop-blur-xl rotate-[-12deg]"
      >
        🎉 SWAP !
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute right-6 top-8 z-40 rounded-2xl border-2 border-rose-400 bg-gradient-to-r from-rose-500 to-pink-500 px-5 py-2.5 text-base font-black uppercase tracking-widest text-white shadow-lg backdrop-blur-xl rotate-[12deg]"
      >
        💨 PASSER
      </motion.div>
      ...
      <div className="relative h-full w-full bg-background overflow-hidden">
        ...
        <div className="absolute inset-x-4 top-6 z-30 flex items-center justify-between gap-3 pointer-events-none">
          <div className="flex items-center gap-1.5 rounded-full border border-white/20 bg-black/40 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-white backdrop-blur-xl">
          ...
          <div className="flex items-center gap-2 rounded-full border border-white/20 bg-black/40 px-3 py-1.5 backdrop-blur-xl">
            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-tr from-emerald-400 to-teal-500 text-[10px] font-black text-white">
        ...
        <div className="absolute inset-x-4 bottom-4 z-30 pointer-events-none">
          <div className="rounded-[30px] border border-white/20 bg-black/65 p-5 backdrop-blur-2xl shadow-xl">
            <div className="flex items-end justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <h2 className="mt-1 line-clamp-2 font-display text-2xl font-black leading-tight tracking-tight text-white">
                  {item.title}
                </h2>
              </div>

              {/* Swaps Price Pill */}
              <div className="shrink-0 rounded-[22px] border border-emerald-400/40 bg-gradient-to-br from-emerald-500/30 via-teal-500/20 to-transparent px-4 py-3 text-right backdrop-blur-md shadow-md">
                <span className="block text-[8px] font-black uppercase tracking-widest text-emerald-300">
                  Valeur
                </span>
